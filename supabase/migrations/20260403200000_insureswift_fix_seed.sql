-- ============================================================
-- InsureSwift Fix Migration — Idempotent Full Schema + Seed
-- ============================================================
...
-- 8. SEED DATA
-- Uses ON CONFLICT (email) DO NOTHING so re-runs are safe
DO $$
DECLARE
  admin_uuid UUID;
  claimant_uuid UUID;
...
BEGIN
  ...
  -- ── Admin user ──────────────────────────────────────────────
  SELECT id INTO admin_uuid FROM auth.users WHERE email = 'admin@insureswift.com' LIMIT 1;
  ...
  INSERT INTO public.user_profiles (id, email, full_name, role, lic_id)
  VALUES (admin_uuid, 'admin@insureswift.com', 'Arvind Kumar', 'admin', 'LIC-ADM-001')
  ON CONFLICT (id) DO NOTHING;

  -- ── Claimant user ────────────────────────────────────────────
  SELECT id INTO claimant_uuid FROM auth.users WHERE email = 'rahul@example.com' LIMIT 1;

  IF claimant_uuid IS NULL THEN
    claimant_uuid := gen_random_uuid();
    INSERT INTO auth.users (
      id, instance_id, aud, role, email, encrypted_password, email_confirmed_at,
      created_at, updated_at, raw_user_meta_data, raw_app_meta_data,
      is_sso_user, is_anonymous, confirmation_token, confirmation_sent_at,
  ...
  VALUES (claimant_uuid, 'rahul@example.com', 'Rahul Mehta', 'claimant')
  ON CONFLICT (id) DO NOTHING;

  -- ── Policies ─────────────────────────────────────────────────
  pol_mtr_uuid  := gen_random_uuid();
  pol_hlt_uuid  := gen_random_uuid();
  pol_prp_uuid  := gen_random_uuid();
  pol_mtr2_uuid := gen_random_uuid();
  pol_hlt2_uuid := gen_random_uuid();
  ...
  -- Re-fetch policy UUIDs in case they already existed
  SELECT id INTO pol_mtr_uuid FROM public.policies WHERE policy_number = 'LIC-MTR-00291' LIMIT 1;
  SELECT id INTO pol_hlt_uuid FROM public.policies WHERE policy_number = 'LIC-HLT-00184' LIMIT 1;
  SELECT id INTO pol_prp_uuid FROM public.policies WHERE policy_number = 'LIC-PRP-00047' LIMIT 1;

  -- ── User policy assignments ───────────────────────────────────
  INSERT INTO public.user_policies (user_id, policy_id, policy_number, start_date, end_date, vehicle_or_asset, assigned_by)
  VALUES
    (claimant_uuid, pol_mtr_uuid, 'LIC-MTR-00291', '2026-03-15', '2027-03-14', 'Honda City – HR26DK4421',        admin_uuid),
    (claimant_uuid, pol_hlt_uuid, 'LIC-HLT-00184', '2025-06-01', '2026-05-31', 'Family Floater – 4 Members',     admin_uuid),
    (claimant_uuid, pol_prp_uuid, 'LIC-PRP-00047', '2025-01-10', '2026-01-09', 'Flat 4B, Sector 21, Gurgaon',    admin_uuid)
